class FoodsData {
  final String title;
  final String image;
  final String categories;
  final int? quantity;

  FoodsData( 
      {required this.title, this.quantity,required this.image, required this.categories});
}

List<FoodsData> fruitsVegiesData = [
  FoodsData(
      title: 'Apples',
      image: 'assets/images/trans_apple.png',
      categories: 'FruitsVegs',
      quantity: 0
      ),
  FoodsData(
      title: 'Avocado',
      image: 'assets/images/trans_avocado.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Banana',
      image: 'assets/images/trans_banana.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Beetroot',
      image: 'assets/images/trans_beetroot.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Bell Pepper',
      image: 'assets/images/trans_bell_pepper.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Berries',
      image: 'assets/images/trans_berry.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Broccoli',
      image: 'assets/images/trans_broccoli.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Cabbage',
      image: 'assets/images/trans_cabbage.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Carrots',
      image: 'assets/images/trans_carrots.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Cauliflower',
      image: 'assets/images/trans_cauliflower.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Cucumber',
      image: 'assets/images/trans_cucumber.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Grapes',
      image: 'assets/images/trans_grapes.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'GreenPeas',
      image: 'assets/images/trans_greenpeas.png',
      categories: 'FruitsVegs',quantity: 0),
  FoodsData(
      title: 'Lemon',
      image: 'assets/images/trans_lemon.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Lettuce',
      image: 'assets/images/trans_lettuce.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Mango',
      image: 'assets/images/trans_mango.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Okro',
      image: 'assets/images/trans_okro.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Onion',
      image: 'assets/images/trans_onion.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Orange',
      image: 'assets/images/trans_orange.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Pepper',
      image: 'assets/images/trans_pepper.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Pineapple',
      image: 'assets/images/trans_pineapple.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Tomatoes',
      image: 'assets/images/trans_tomatoe.png',
      categories: 'FruitsVegs',
      quantity: 0),
  FoodsData(
      title: 'Watermelon',
      image: 'assets/images/trans_watermelon.png',
      categories: 'FruitsVegs',
      quantity: 0),
];


List<FoodsData> meats = [
  FoodsData(
    title: 'Beef', 
    image: 'assets/images/beef.png', 
    categories: 'Proteins'),
    FoodsData(
    title: 'Lamb', 
    image: 'assets/images/trans_lamb.png', 
    categories: 'Proteins'),
    FoodsData(
    title: 'Goat Meat', 
    image: 'assets/images/trans_goat_meat.png', 
    categories: 'Proteins'),
    FoodsData(
    title: 'Pork', 
    image: 'assets/images/trans_pork.png', 
    categories: 'Proteins'),
    FoodsData(
    title: 'Catfish', 
    image: 'assets/images/catfish.png', 
    categories: 'Proteins'),
    FoodsData(
    title: 'Chicken', 
    image: 'assets/images/trans_chicken.png', 
    categories: 'Proteins'),
    FoodsData(
    title: 'Crab', 
    image: 'assets/images/trans_crab.png', 
    categories: 'Proteins'),
    FoodsData(
    title: 'Eggs', 
    image: 'assets/images/eggs.png', 
    categories: 'Proteins'),
    FoodsData(
    title: 'Prawns', 
    image: 'assets/images/trans_prawns.png', 
    categories: 'Proteins'),
    FoodsData(
    title: 'Lobster', 
    image: 'assets/images/trans_lobster.png', 
    categories: 'Proteins'),
    FoodsData(
    title: 'Salmon', 
    image: 'assets/images/trans_salmon.png', 
    categories: 'Proteins'),
    FoodsData(
    title: 'Tilapia', 
    image: 'assets/images/trans_tilapia.png', 
    categories: 'Proteins'),
    FoodsData(
    title: 'Turkey', 
    image: 'assets/images/trans_turkey.png', 
    categories: 'Proteins'),
   
];
